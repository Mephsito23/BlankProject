...
+ (instancetype)sheetWithTitle:(NSString *)title buttonTitles:(NSArray *)titles                       buttonImgs:(NSArray *)images redButtonIndex:(NSInteger)buttonIndex delegate:(id<LCActionSheetDelegate>)delegate {
    
    return [[self alloc] initWithTitle:title buttonTitles:titles                      buttonImgs:images redButtonIndex:buttonIndex delegate:delegate];
}
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (title) {
            ...
            // 标题
//            UIImageView *label = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_SIZE.width, BUTTON_H)];
//            label.image = [UIImage imageNamed:title];
